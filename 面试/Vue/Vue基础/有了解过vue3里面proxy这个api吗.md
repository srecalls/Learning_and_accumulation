`Proxy API`对应的`Proxy`对象是[ES2015](https://link.zhihu.com/?target=https%3A//www.ecma-international.org/ecma-262/6.0/%23sec-proxy-objects)就已引入的一个原生对象，用于定义基本操作的自定义行为（如属性查找、赋值、枚举、函数调用等）。
...
`Reflect.set()`用于修改数组的值，参考[Reflect](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect)，但是对于多层对象嵌套问题，需要经过一定的处理：
...
当然，上面这段代码只是Vue3中reactive的一个缩影，更多的细节可以浏览相关[源码](https://link.zhihu.com/?target=https%3A//github.com/vuejs/vue-next/tree/a5b4332c69146de569ad328cac9224c3cded15c9/packages/reactivity/src)来了解。
