...
6. 
![[Pasted image 20230608151746.png]]
...
![[Pasted image 20230608151842.png]]
三、断点续传
断点续传可以分为两种场景：
...
![[Pasted image 20230608162612.png]]
# 大文件上传
## 一、项目初始化
### 1、项目初始化
...
实现在刷新后保留原始文件的断点续传功能是比较复杂的，涉及到文件的分片上传、记录上传进度、以及在刷新后恢复上传的过程。下面是一个基本的思路和示例代码：

1. 文件分片上传：将大文件切分成多个小的文件块（chunk），并使用文件上传接口将这些文件块逐个上传到服务器。

2. 记录上传进度：在上传过程中，记录已经成功上传的文件块信息，包括文件块索引、上传状态等。可以使用浏览器的本地存储（localStorage 或 sessionStorage）来保存这些信息。

3. 恢复上传：在页面刷新后，检查本地存储中是否存在上传进度信息。如果有，则根据记录的文件块信息，继续上传未完成的文件块。

下面是一个简单的示例代码，演示如何实现断点续传并在刷新后保留原始文件：
[[3.Cookie、LocalStorage、SessionStorage区别]]
```javascript
// 保存上传进度信息到本地存储
function saveUploadProgress(progress) {
  localStorage.setItem('uploadProgress', JSON.stringify(progress));
}

// 获取本地存储中的上传进度信息
function getUploadProgress() {
  var progress = localStorage.getItem('uploadProgress');
  if (progress) {
...
function uploadFile(file) {
  var chunks = splitFileIntoChunks(file); // 将文件切分成多个块
  var currentChunkIndex = 0;
  var progress = getUploadProgress();

  // 恢复上传进度
  if (progress) {
    currentChunkIndex = progress.currentChunkIndex;
  } else {
    progress = { currentChunkIndex: 0, uploadedChunks: [] };
  }
  ...
  function uploadNextChunk() {
    if (currentChunkIndex >= chunks.length) {
      ...
      clearUploadProgress(); // 清除上传进度信息
      return;
    }

    var chunk = chunks[currentChunkIndex];
    if (progress.uploadedChunks.includes(currentChunkIndex)) {
      // 已经上传过该块，跳过
      currentChunkIndex++;
      uploadNextChunk();
      return;
    ...
    uploadChunk(chunk)
      .then(() => {
        // 上传成功
        progress.uploadedChunks.push(currentChunkIndex);
        progress.currentChunkIndex++;
        saveUploadProgress(progress); // 保存上传进度信息
        currentChunkIndex++;
        uploadNextChunk();
      })
      .catch((error) => {
...
在这个示例中，我们使用 `uploadFile` 函数来上传文件。文件会被切分成多个块，并使用 `uploadChunk` 函数来模拟上传每个块的过程。上传进度信息会根据每个成功上传的块进行记录，并保存在本地存储中。

在刷新后，我们可以通过调用 `getUploadProgress` 函数来获取之前保存的上传进度信息。如果存在上传进度信息，则根据记录的块信息继续上传未完成的块。

需要注意的是，这只是一个简单的示例，实际的断点续传实现会更加复杂，需要处理更多的边界情况和错误处理。此外，还需要考虑上传过程中的并发控制和服务器端的支持。
...
原文链接：https://blog.csdn.net/lan7798/article/details/127550233