...
组件复用详解见[组件复用](https://www.cxymsg.com/guide/abstract.html)
...
异步渲染模式就是不阻塞当前线程，继续跑。在视频里可以看到所有的输入，表上都会是绿色的。
...
4. 首先，用户（通过View）发出Action，发出方式就用到了dispatch方法。
5. 然后，Store自动调用Reducer，并且传入两个参数：当前State和收到的Action，Reducer会返回新的State
6. State一旦有变化，Store就会调用监听函数，来更新View。

到这儿为止，一次用户交互流程结束。可以看到，在整个流程中数据都是单向流动的，这种方式保证了流程的清晰。
![[React面试题(持续更新中)-3.png]]
## react-redux是如何工作的?
